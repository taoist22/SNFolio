...
Recovery from an oversized sync, and the PDF note block (user report 2026-09-22; maintainer decision: build 1, 2, 3):
- User report: an iCloud sync made SNFolio unusable; uninstalling did not clear it (data lives in PluginHost storage) and there was no reset. Reply sent with the backup-restore workaround.
- Calendars & Sync now has 'Remove Synced Events from SNFolio' (clears cached CalDAV events + push state; keeps own events, tasks, PARA, notes; server untouched) and 'Remove Calendar Account…' (keep events / remove events), both behind confirmations.
- Workspace Backup panel now has 'Reset SNFolio…': writes a verified backup, then restores an empty workspace (defaults for settings). Notes never touched; sync paused afterwards as with any restore.
- Daily/event/task note creation now explains the device's 102 'not allowed to use this API': a note cannot be created or appended while a PDF is open (measured 2026-09-18; not fixable in-plugin). CT reproduced and confirmed the cause today.
- Automated: 767 tests pass (new: removeSyncedEvents, emptyWorkspaceData validity, resetWorkspace backs up then empties, PDF message mapping); typecheck passes; lint zero errors.
- Device checks: connect iCloud → sync → Remove Synced Events → calendar empties, own events/tasks/PARA remain, Sync Now brings them back; Remove Calendar Account (both options); Reset SNFolio → backup path shown → SNFolio empty, notes still in Files, restore that backup → everything back; open a PDF, open SNFolio, Create this Day's Journal → new message; repeat in a note → works.
- NOT built (deferred): sync window guard/setting (still 30 days back, 365 ahead) and a recovery mode for a workspace too large to render — needs measurement first.
- Build 2026-09-22 08:13 (version unchanged 0.1.27 / 72): build/generated and build/outputs removed first; 767 tests pass; fresh build and package validation pass. Bundle confirmed to contain Remove Synced Events, Remove Calendar Account, Reset SNFolio and the PDF note message (the ellipsis is escaped in the bundle, so those strings verify without it); native package still has writeAutoBackupFile.
- Fixes after CT's Nomad test of Reset (2026-09-22): (1) a reset no longer pauses synchronization — the pause is a restore-only review step, and it silently blocked 'Connect & Test iCloud CalDAV' with a message the user could not see; (2) the workspace notice at the top now has a Dismiss button (the panel never unmounts, so it previously stayed for the whole session, even after closing the plugin); (3) the paused-sync banner now carries its own 'Allow Reconnection' button and states what reconnecting may send, instead of pointing at Help & Setup (that button now shares the same handler). 768 tests (new: reset leaves sync usable, restore still pauses). Device checks: Reset on Nomad → notice can be dismissed and stays gone after closing/reopening the plugin → connect iCloud immediately → sync works; restore a backup → banner appears with Allow Reconnection → tap → connect works.
- Build 2026-09-22 08:37 (version unchanged 0.1.27 / 72): build/generated and build/outputs removed first; 768 tests pass; fresh build and package validation pass. Bundle confirmed to contain Allow Reconnection, the dismissible notice, and the reset/removal controls.

Wording and backup-list fixes (maintainer approved 2026-09-22, after Nomad testing):
- Connection message reworded: 'Connected to iCloud CalDAV. New events you create go to "Calendar". 7 calendars found.' (the count was read as events written to the server). A successful connection now also PULLS events immediately and reports 'Read N event(s)' — before, connecting only pushed, so an account looked empty until Sync Now.
- Calendars & Sync: confirmations now headed 'Remove synced events' / 'Disconnect account'; a hint under the buttons states the difference; both actions clear the stale sync summary; the disconnect message says Sync Now will report no sources until an account is connected again.
- Backup restore no longer uses the system file picker for SNFolio's own folder. The picker refuses paths outside the plugin whitelist and warned 'not allowed to access sdcard path outside whitelist: …/Export/SNFolio Backups/…' even though the plugin reads the file through its own native module. Choose Backup to Restore… now lists Export/SNFolio Backups (newest first, with date and size) via CalendarFile.listFolderEntries, which now also returns modified and size; Browse… keeps the picker for backups kept elsewhere.
- Automated: 769 tests pass (new/updated: backup listing from the native module and never via the picker, panel list flow); typecheck passes; lint zero errors.
- Device checks: Choose Backup to Restore… lists both manual and auto backups with dates, no whitelist warning; tap one → review → restore; Browse… still works; connect an account → events appear without pressing Sync Now; remove synced events → hint and message wording; disconnect → message mentions Sync Now reporting no sources.
- Build 2026-09-22 09:24 (version unchanged 0.1.27 / 72): build/generated and build/outputs removed first; 769 tests pass; fresh build and package validation pass. Bundle confirmed to contain the reworded connection message, the remove/disconnect headings and the backup list ('Tap a backup to review it', Browse); native package has listFolderEntries with the new modified/size fields and writeAutoBackupFile.

Recurring-event performance (measured 2026-09-22, after the user report guessed 'too many events'):
- Measured with a throwaway benchmark (not committed) on the developer machine, timing expandEventsForDate over 30 days: 10,000 one-off events cost ~19 ms, but 500 weekly-recurring events cost ~355 ms, 2,000 ~1.0 s, 5,000 ~2.5 s, 10,000 ~5.0 s. Cause: every day drawn asked every recurring event whether it occurs, and answering walks the rule forward from its DTSTART, so cost = recurring events × days drawn × event age. Volume alone was NOT the driver; recurrence was.
- Fix: expandEventsForDate now caches each recurring event's occurrences for the month around the requested day (WeakMap keyed by the event object, so parsed/synced replacements never reuse a stale expansion) and filters that window per day.
- After: 500 → ~34 ms, 2,000 → ~93 ms, 5,000 → ~233 ms, 10,000 → ~454 ms for a month; redrawing the same month ~3× cheaper again (11–172 ms). Roughly 10–11× faster.
- Regression cover is deterministic, not timing-based: counting reads of a field only touched while a rule is walked shows one walk per month instead of one per day, with identical results, plus a test that an edited rule is not served from the cache.
- Automated: 771 tests pass; typecheck passes; lint zero errors. Device check: open a month with a busy recurring calendar and page between months; compare responsiveness with 0.1.27.

Second performance pass (2026-09-22, after CT confirmed the 'busy' test calendar — 300 weekly series + 800 one-off — was still slow on device):
- Measured pipeline on the busy shape: filterEvents 0 ms, month grid 38 ms (34 ms for the same month again), PARA 60-day list 34 ms, one day 1 ms with 47 events. The grid asked all 1,100 events about each of its 42 cells (~46,000 event visits per redraw).
- New expandEventsByDay(events, from, to) expands each event once across a range and files occurrences under every day they touch (overnight, multi-day, zero-length and midnight-end cases covered by an equivalence test against expandEventsForDate for every day of a month). generateMonthGrid, the weekly-review week and the PARA 60-day list now use it.
- Month grid on the busy shape after: 11–18 ms (was 38), and no longer re-walking rules per cell.
- Note-existence sweep: results are remembered per path for the session (cleared at the start of a sweep whenever refreshState or the notes folder changes, so a note just created is seen). Previously every day change cost one native exists() per event without a recorded note — up to 47 file calls per day on the busy calendar, repeated on every revisit. Failed checks are not remembered.
- Benchmark kept as src/domain/calendarPerformance.test.ts, asserting work counts rather than milliseconds (recurring must not cost dramatically more than plain; one pass must walk rules fewer times than cell-by-cell; identical occurrences either way).
- Automated: 776 tests pass; typecheck passes; lint zero errors. Device check: import snfolio-test-busy.ics, page through months and switch days repeatedly, then remove the imported calendar.
- Build 2026-09-22 09:56 (version unchanged 0.1.27 / 72): build/generated and build/outputs removed first; 776 tests pass; fresh build and package validation pass. Performance changes add no new user-visible strings, so the bundle was verified by the earlier labels plus its new size; the real check is on-device paging with snfolio-test-busy.ics.

On-device screen timings (2026-09-22): CT reports the second performance pass is only 'minimally faster' on device, so the remaining cost is not visible from developer-machine measurements. Added a diagnostic: settings.showScreenTimings (Help & Setup toggle, off by default, backed up) turns on a readout under the header showing the last six samples. Instrumented: 'month build' (generateMonthGrid, with event count), 'month draw' (commit-to-paint for the month grid, with occurrences shown), 'day filter', 'day build', and 'screen draw' (whole panel, with view mode). Recording is two Date.now() calls and is skipped entirely when off. 780 tests pass (new: perfTrace behaviour — off records nothing, newest-first cap of 6, listener notification, clearing on switch-off); the busy-calendar test now counts rule walks instead of milliseconds, because the timing assertion proved flaky under a parallel suite run. Device use: load snfolio-test-busy.ics, switch the toggle on, page a month, change days, and read off build vs draw.
- Build 2026-09-22 10:06 (version unchanged 0.1.27 / 72): build/generated and build/outputs removed first; 780 tests pass; fresh build and package validation pass. Bundle confirmed to contain the timings toggle and every instrumented label (month build/draw, day build, screen draw).
- Timings are also written to a file: Help & Setup → 'Save Timings to Export…' writes Export/SNFolio Diagnostics/screen-timings-<stamp>.txt (header: screen size, events/tasks/projects loaded; then one tab-separated line per measurement, oldest first). The readout still shows the last six; the log keeps up to 400 and is cleared when tracing is switched on. Refuses with a hint when nothing has been recorded. 781 tests pass.

Device measurements and the fixes they led to (2026-09-22/23, Nomad 749x998, 1112 events from snfolio-test-busy.ics; log saved via the new diagnostic):
- Measured on device: month build 560-1050 ms, month draw ~230 ms, screen draw 1150-1700 ms (first 3410 ms), day filter ~140 ms. The developer machine is roughly 40x faster, which is why earlier changes felt 'minimally faster' there.
- DIAGNOSTIC FLAW found in its own first log: the readout re-rendered on every sample, which rebuilt the month, which recorded another sample — month build fired ~10x/second with unchanged data. The readout now refreshes on a 2-second timer; samples are still collected as they happen.
- filterEvents ran on every render of several screens and handed MonthGridView a NEW array each time, so nothing downstream could be reused. Now filtered once (useMemo) and shared by the month grid, week view, day view and tomorrow summary.
- Dedupe fingerprints use toLowerCase instead of toLocaleLowerCase for the in-session comparison; the stored hide identity keeps the locale-aware form so ids saved by older versions still match.
- generateMonthGrid results are cached per events array (WeakMap) and month, up to 12 months, with today's date in the key. Paging back to a month is now free; a sync or edit replaces the events array and drops the cache with it.
- Developer-machine timings after: filter 1 ms (was 0-ish but per render), month build 11 ms first view, 0 ms paged back, 15 ms next month.
- Automated: 783 tests pass (new: grid cache reuse, cache dropped for a new events array, today's date in the key); typecheck passes; lint zero errors.
- Device check: with the busy calendar loaded, page months back and forth and switch days; save timings again and compare month build/draw and screen draw with this log.
- Build 2026-09-22 15:39 (version unchanged 0.1.27 / 72): build/generated and build/outputs removed first; 783 tests pass; fresh build and package validation pass. Bundle contains the new 'filter events' measurement alongside the earlier labels.

Third performance pass, from the second device log (2026-09-23 01:40, same Nomad and 1112 events):
- Confirmed working: month build 0 ms when paging back (grid cache), and the diagnostic no longer drives its own re-renders.
- Remaining problem 1: building a month for the FIRST time still cost 777-1020 ms. Cause: rules with COUNT were enumerated day by day from DTSTART; only simple UTC daily rules could skip ahead. The test data (and Canvas/Brightspace/iCloud feeds) use FREQ=WEEKLY;COUNT=n, so ~300 series each walked up to a year of days per month view. Fast-forward now also covers rules that produce exactly one occurrence per period and can never skip one: WEEKLY without BYDAY, MONTHLY without BYDAY/BYMONTHDAY when the day is <= 28, and YEARLY except 29 February. Those exclusions matter: a monthly 31st or a yearly 29 February rule skips periods, so counting periods would consume COUNT too fast and lose occurrences at the tail.
- Equivalence cover: for ten rule shapes (weekly, every other week, weekly BYDAY, monthly 15th, monthly 31st, BYMONTHDAY=29, yearly, yearly 29 Feb, daily, all-day weekly) the occurrences in windows at the start, middle, past-end of the series must match those taken from a window beginning at DTSTART. Developer-machine first build now 8-11 ms per month (was ~38 before any of this work, and the device showed 777-1020 ms).
- Remaining problem 2: month draw (~240 ms) ran six or seven times per interaction because the grid re-rendered whenever anything else on the panel changed. MonthGridView is now React.memo'd and its two callbacks are useCallback'd, so it re-renders only when its own inputs change.
- Automated: 793 tests pass; typecheck passes; lint zero errors.
- Device check: repeat the same run and compare — first view of a new month should drop sharply, and month draw should appear about once per interaction instead of six times.
- Build 2026-09-22 15:49 (version unchanged 0.1.27 / 72): build/generated and build/outputs removed first; 793 tests pass; fresh build and package validation pass. Contains the COUNT fast-forward and the memoised month grid.

Fourth performance pass, from the third device log (2026-09-23 01:54):
- Result of pass three on device: month build 777-1020 → 635 ms (less than predicted), month draw still SEVEN times per month change, so the React.memo on MonthGridView was not taking effect.
- Cause found: both context providers (EventDesignationsContext, LinkedFilePathsContext) built their value inline on every render — a map over all 1112 events plus every task, calling getMembership and resolveEventDesignation for each. That is O(events) work per render AND a new object identity, which re-rendered every consumer (LinkedFileMarker in each row, every month cell), defeating the memo. Both are now useMemo'd on [allParsedEvents, tasks, projects, membershipRevision] and [membershipRevision, refreshState].
- Second change: a weekly rule without BYDAY recurs on one weekday, so the walk now steps a whole interval of weeks once it is on an occurrence instead of checking the six days between. With ~300 such series a month view walked ~13,000 days; now ~2,000.
- Developer-machine first build per month: 8-11 ms → 4-5 ms (the device is where this matters; it walked the same days ~70x slower).
- Automated: 793 tests pass, including the ten-shape COUNT equivalence tests that cover weekly striding; typecheck passes; lint zero errors.
- Device check: same run again — month build on a new month, and how many month draw lines appear per month change (should be about one).

Fifth pass — the day view (from the fourth device log, 2026-09-23 02:06):
- Fourth pass worked: month build 635 → 383-385 ms, month draw 7 → 2 per month change, the repeated redraws 330 → ~95 ms (the context memoisation), first screen draw 1348-2382 → 1025-1546 ms.
- Day view became the worst screen: one draw of 590-720 ms then five or six more at 370-530 ms per day change (~2.5 s), while its day build was only ~21 ms — all rendering, not calculation. Several state updates land separately on a day change (the day's events, the note sweep, the journal check), and DayScheduleGrid re-rendered for each because its callbacks were rebuilt every render.
- DayScheduleGrid is now React.memo'd, and its four callbacks have fixed identities that call whichever handler version is current (a ref), so its props change only when its data does — the handlers themselves depend on half the screen, so freezing them with dependency lists would have been fragile.
- Also memoised, both of which ran on every render: the tomorrow summary (which expanded tomorrow's events from all 1112) and the day's task sections.
- Automated: 793 tests pass; typecheck passes; lint zero errors.
- Device check: same run; watch the agenda/day screen draw lines per day change.

Sixth pass — repeated state with identical contents (from the fifth device log, 2026-09-23 02:24):
- Day view after the memoisation: a day change went from ~2.5 s to ~1.8 s, and three of its redraws dropped to 170-210 ms. Two expensive ones remained (~490 and ~360 ms).
- Cause: two sweeps replace state with a fresh object every time they run, even when they found exactly what they found last time — the note-existence sweep (eventNotePaths) and the month's journal-note check (dailyNoteDates). A new object identity re-renders the panel, and with it the month grid (dailyNoteDates is one of its props) and the day schedule (notePaths). With the test calendar nothing ever changes, because none of those events have notes.
- Both now keep the previous value when the contents match, through sameNotePaths / sameKeys in src/screens/stateEquality.ts (extracted so they are testable).
- Automated: 795 tests pass; typecheck passes; lint zero errors.
- Device check: same run; the second month draw per month change and the ~490 ms day redraw should disappear when nothing has actually changed.

Diagnostic hidden before release (maintainer decision 2026-09-23): the timings toggle, the Save Timings to Export button and the README section are removed. perfTrace, PerfReadout, usePaintTiming and every timePerf call stay in the build (inert unless settings.showScreenTimings is true, which nothing now sets), so a later release can expose them again without rebuilding the instrumentation. Rationale: users rarely report what they did, and the measured issues have been fixed. 795 tests pass; typecheck passes; lint zero errors.
- Maintainer confirms all three were device-tested: Remove Account — Keep Events, the restore banner's Allow Reconnection, and the PDF note message.
- Release build 2026-09-23: version 0.1.28 / 73. build/generated and build/outputs removed first; 795 tests pass, typecheck passes, lint zero errors; fresh build and package validation pass; bundle contains the recovery controls, the PDF message and the backup list, and no timings toggle; native package still has writeAutoBackupFile.
- Documentation after 0.1.28 (no version bump, docs only): README now covers the restore banner's Allow Reconnection (and that Connect & Test refuses until it is tapped) and the Dismiss button on the notice; docs/CALENDAR_CONNECTIONS.md step 6 describes the new connection message and that connecting reads events immediately; docs/TROUBLESHOOTING.md gains 'A note cannot be created while a PDF is open' and 'SNFolio is slow, or a sync brought in too much' (reinstalling does not clear the data; the three ways to clear it).
