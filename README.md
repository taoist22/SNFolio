...
- **Projects** are actionable outcomes with due dates, progress, tasks, assigned events, linked meeting notes, and a folder of supporting files. Each Project card separates open and upcoming items from completed tasks. To keep the list short, **Open & Upcoming** shows anything overdue plus this week and next week (by your calendar's week start); a line such as **+ 7 later · 1 with no date ›** opens the Project for the rest. Tap **▾** beside a Project's name to collapse its card to one line (for example *1 overdue · 2 due in the next two weeks · 3 events*), or **Collapse All** / **Expand All** at the top. SNFolio remembers which cards are collapsed. **Mark Complete** records that a Project is done; **Archive** removes unfinished work from the active view without claiming it was completed. Both are under **Project Actions…** at the bottom of the Project.
...
- After a restore, calendar feeds and account synchronization stay **paused** until you review the restored items and reconnect, so old edits or queued deletions are not sent to your server by surprise. A banner at the top of the screen says so and carries **Allow Reconnection**; tap it once you have looked over the restored items, then reconnect the account in **Calendars & Sync**. The same button is in **Help & Setup**. Until you do, **Connect & Test** refuses, and Sync Now does nothing.
- The message after a restore or reset has a **Dismiss** button. SNFolio's panel stays loaded between uses, so closing it does not clear that message on its own.
